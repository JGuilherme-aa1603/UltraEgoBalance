...
public final class BalanceNetwork {
    ...
    public static void syncDestruction(ServerPlayer player) {
        ...
        boolean egoRequirementBypassed = InstinctTechnique.destructionUnlocked(player);
        CHANNEL.send(PacketDistributor.PLAYER.with(() -> player), new DestructionSyncPacket(
                ...
                egoRequirementBypassed ? 0.0f : BalanceConfig.HAKAI_REQUIRED_EGO.get().floatValue(),
                egoRequirementBypassed ? 0.0f : BalanceConfig.SPHERE_REQUIRED_EGO.get().floatValue(),
